# Create a raw transaction with an amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# raw_tx=""

recipient_address="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"
raw_tx=""

utxo_txid=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx" 2>/dev/null | jq -r '.txid')
utxo_vout_1=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx" 2>/dev/null | jq -r '.vout[] | select(.n == 0) | .value')
utxo_vout_2=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx" 2>/dev/null | jq -r '.vout[] | select(.n == 1) | .value')
utxo_vout=$(echo "$utxo_vout_1 + $utxo_vout_2" | bc)

bitcoin-cli -regtest createrawtransaction "[{\"txid\":\"$utxo_txid\",\"vout\":$utxo_vout}]" "{\"$recipient_address\":0.2}"